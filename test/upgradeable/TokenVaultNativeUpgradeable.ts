...
import { TokenVaultNativeUpgradeableInstance, MultiSigWalletInstance, TokenVaultNativeUpgradeableTestInstance, USDPUpgradeableInstance } from '../../types/truffle-contracts';
import TOKEN_VAULT_NATIVE_UPGRADEABLE_JSON from '../../build/contracts/TokenVaultNativeUpgradeable.json';
...
const TokenVaultNativeUpgradeable = artifacts.require('TokenVaultNativeUpgradeable');
const multiSigWallet = artifacts.require('MultiSigWallet');
const USDPUpgradeable = artifacts.require('USDPUpgradeable');
const TokenVaultNativeUpgradeableTest = artifacts.require('TokenVaultNativeUpgradeableTest');

const web3 = new Web3(new Web3.providers.HttpProvider(config.AVAX.localSubnetHTTP));
const TOKEN_VAULT_UPGRADEABLE_ABI = TOKEN_VAULT_NATIVE_UPGRADEABLE_JSON.abi as AbiItem[];
...
contract.skip('TokenVaultNativeUpgradeable.sol', ([owner, account1, account2, account3, account4, account5, account6, account7, account8, account9]) => {
    let tokenVaultNativeUpgradeableInstance: TokenVaultNativeUpgradeableInstance;
    let multiSigWalletInstance: MultiSigWalletInstance;
    let USDPUpgradeableInstance: USDPUpgradeableInstance;
    let tokenVaultNativeUpgradeableTestInstance: TokenVaultNativeUpgradeableTestInstance;
    let TokenVaultNativeUpgradeableContract: any;
    ...
    beforeEach(async () => {
        ...
        tokenVaultNativeUpgradeableInstance = (await deployProxy(TokenVaultNativeUpgradeable as any, [account1], {
            initializer: 'initialize',
        })) as TokenVaultNativeUpgradeableInstance;
        await tokenVaultNativeUpgradeableInstance.transferOwnership(multiSigWalletInstance.address);

        TokenVaultNativeUpgradeableContract = new web3.eth.Contract(TOKEN_VAULT_UPGRADEABLE_ABI, tokenVaultNativeUpgradeableInstance.address);
    ...
    it('allows a user to pay for a specific opId', async () => {
        ...
        await localExpect(tokenVaultNativeUpgradeableInstance.payForOpId(opId, { from: account2, value: amountToPay })).to.eventually.be.fulfilled;
        const amountPaid = (await tokenVaultNativeUpgradeableInstance.getUserPaymentAmountForOpId(account2, opId)).toString();
    ...
    it('reverts if the user tries to call the pay function without sending any PFT', async () => {
        const opId = '123';
        await localExpect(tokenVaultNativeUpgradeableInstance.payForOpId(opId, { from: account2 })).to.eventually.be.rejected;
    });
    ...
    it('only allows an opId to be paid for once', async () => {
        ...
        const amountToPay = '1000';
        await localExpect(tokenVaultNativeUpgradeableInstance.payForOpId(opId, { from: account2, value: amountToPay })).to.eventually.be.fulfilled;
        await localExpect(tokenVaultNativeUpgradeableInstance.payForOpId(opId, { from: account2, value: amountToPay })).to.eventually.be.rejected;
    });

    it('allows only the contract owner to refund a user', async () => {
        const opId = '123';
        const amountToPay = '1000';
        const balanceOfPFTVaultBeforePayment = (await web3.eth.getBalance(tokenVaultNativeUpgradeableInstance.address)).toString();
        await localExpect(tokenVaultNativeUpgradeableInstance.payForOpId(opId, { from: account2, value: amountToPay })).to.eventually.be.fulfilled;
        let amountPaid = (await tokenVaultNativeUpgradeableInstance.getUserPaymentAmountForOpId(account2, opId)).toString();
        expect(amountPaid).to.equal(amountToPay);
        const balanceOfPFTVaultAfterPayment = (await web3.eth.getBalance(tokenVaultNativeUpgradeableInstance.address)).toString();
        ...
        await localExpect(tokenVaultNativeUpgradeableInstance.issueFullRefund(account2, opId)).to.eventually.be.rejected;

        // Refund issued by the owner
        let data = TokenVaultNativeUpgradeableContract.methods.issueFullRefund(account2, opId).encodeABI();
        await multiSigWalletInstance.submitTransaction(tokenVaultNativeUpgradeableInstance.address, 0, data, { from: owner });
        let txId = await getTxIdFromMultiSigWallet(multiSigWalletInstance);
        await multiSigWalletInstance.confirmTransaction(txId, { from: account1 });

        amountPaid = (await tokenVaultNativeUpgradeableInstance.getUserPaymentAmountForOpId(account2, opId)).toString();
        expect(amountPaid).to.equal('0');
        const balanceOfPFTVaultAfterRefund = (await web3.eth.getBalance(tokenVaultNativeUpgradeableInstance.address)).toString();
    ...
    it('can only be paused/unpaused by the owner (multiSigWallet)', async () => {
        let isPaused = await tokenVaultNativeUpgradeableInstance.paused();
        expect(isPaused).to.be.false;

        // Non-owner account attempting to pause should fail
        await localExpect(tokenVaultNativeUpgradeableInstance.setPaused(true, { from: account1 })).to.be.rejected;

        let data = TokenVaultNativeUpgradeableContract.methods.setPaused(true).encodeABI();
        await multiSigWalletInstance.submitTransaction(tokenVaultNativeUpgradeableInstance.address, 0, data, { from: owner });
        let txId = await getTxIdFromMultiSigWallet(multiSigWalletInstance);
        await localExpect(multiSigWalletInstance.confirmTransaction(txId, { from: account1 })).to.eventually.be.fulfilled;

        isPaused = await tokenVaultNativeUpgradeableInstance.paused();
        expect(isPaused).to.be.true;

        // Non-owner account attempting to unpause should fail
        await localExpect(tokenVaultNativeUpgradeableInstance.setPaused(false, { from: account1 })).to.be.rejected;

        data = TokenVaultNativeUpgradeableContract.methods.setPaused(false).encodeABI();
        await multiSigWalletInstance.submitTransaction(tokenVaultNativeUpgradeableInstance.address, 0, data, { from: owner });
    ...
    it('only allows the contract owner to withdraw PFT', async () => {
        ...
        await localExpect(tokenVaultNativeUpgradeableInstance.withdrawPFT(account8, amountToPay)).to.eventually.be.rejected;

        const accountBalanceOfPFTBefore = (await web3.eth.getBalance(account8)).toString();

        const data = TokenVaultNativeUpgradeableContract.methods.withdrawPFT(account8, amountToPay).encodeABI();
        await multiSigWalletInstance.submitTransaction(tokenVaultNativeUpgradeableInstance.address, 0, data, { from: owner });
    ...
    it('only allows the contract owner to withdraw ERC20 tokens', async () => {
        ...
        let data = USDPUpgradeableContract.methods.mint(tokenVaultNativeUpgradeableInstance.address, amountToMint).encodeABI();
        await multiSigWalletInstance.submitTransaction(USDPUpgradeableInstance.address, 0, data, { from: owner });
        let txId = await getTxIdFromMultiSigWallet(multiSigWalletInstance);
        await multiSigWalletInstance.confirmTransaction(txId, { from: account1 });

        // This should be rejected as account0 is not the contract owner
        await localExpect(tokenVaultNativeUpgradeableInstance.withdrawTokens(USDPUpgradeableInstance.address, account8, amountToMint)).to.eventually.be.rejected;

        const accountBalanceOfUSDPBefore = (await USDPUpgradeableInstance.balanceOf(account8)).toString();

        data = TokenVaultNativeUpgradeableContract.methods.withdrawTokens(USDPUpgradeableInstance.address, account8, amountToMint).encodeABI();
        await multiSigWalletInstance.submitTransaction(tokenVaultNativeUpgradeableInstance.address, 0, data, { from: owner });
        txId = await getTxIdFromMultiSigWallet(multiSigWalletInstance);
        await multiSigWalletInstance.confirmTransaction(txId, { from: account1 });
    ...
    it('allows PFT to be transferred from the vault if the signature is successfully verified', async () => {
        ...
        const domain = {
            ...
            version: '1',
            chainId: 43214,
            verifyingContract: tokenVaultNativeUpgradeableInstance.address,
        };

        const amountToSendToVault = web3.utils.toWei('100', 'ether');
        await tokenVaultNativeUpgradeableInstance.sendTransaction({ from: account9, value: amountToSendToVault });
        ...
        const vaultBalanceBefore = (await web3.eth.getBalance(tokenVaultNativeUpgradeableInstance.address)).toString();
        ...
        await localExpect(tokenVaultNativeUpgradeableInstance.transferFromVault(signature, amountToTransfer, { from: testAccountsData[1].address })).to.eventually.be.fulfilled;

        const vaultBalanceAfter = (await web3.eth.getBalance(tokenVaultNativeUpgradeableInstance.address)).toString();
    ...
    it('reverts if the same signature is used multiple times', async () => {
        ...
        await localExpect(tokenVaultNativeUpgradeableInstance.transferFromVault(signature, amountToTransfer, { from: testAccountsData[1].address })).to.eventually.be.rejected;
    });
    ...
    it("prevents PFT from being transferred if the signed 'recipient' address doesn't match the sender address of the tx", async () => {
        ...
        await localExpect(tokenVaultNativeUpgradeableInstance.transferFromVault(signature, amountToTransfer, { from: testAccountsData[2].address })).to.eventually.be.rejected;
    });
    ...
    it("prevents PFT from being transferred if the signed `amount` doesn't match the tokenId specified by the caller", async () => {
        ...
        await localExpect(tokenVaultNativeUpgradeableInstance.transferFromVault(signature, web3.utils.toWei('0.5', 'ether'), { from: testAccountsData[1].address })).to.eventually
            .be.rejected;
    });
    ...
    it('prevents PFT from being transferred if the signature is tampered with', async () => {
        ...
        await localExpect(tokenVaultNativeUpgradeableInstance.transferFromVault(tamperedSignature, amountToTransfer, { from: testAccountsData[2].address })).to.eventually.be
            .rejected;
    });

    it('only allows the owner to change the PFTVaultTransferSigner', async () => {
        // Should fail since caller is not the owner
        await localExpect(tokenVaultNativeUpgradeableInstance.setPFTVaultTransferSigner(account3, { from: account1 })).to.eventually.be.rejected;

        const data = TokenVaultNativeUpgradeableContract.methods.setPFTVaultTransferSigner(account3).encodeABI();
        await multiSigWalletInstance.submitTransaction(tokenVaultNativeUpgradeableInstance.address, 0, data, { from: owner });
        const txId = await getTxIdFromMultiSigWallet(multiSigWalletInstance);
        await localExpect(multiSigWalletInstance.confirmTransaction(txId, { from: account1 })).to.eventually.be.fulfilled;

        const contractPFTVaultTransferSigner = await tokenVaultNativeUpgradeableInstance.PFTVaultTransferSigner();
    ...
    it("only allows PFT to be transferred if the signer is updated to match the contract's changed PFTVaultTransferSigner", async () => {
        ...
        let data = TokenVaultNativeUpgradeableContract.methods.setPFTVaultTransferSigner(account2).encodeABI();
        await multiSigWalletInstance.submitTransaction(tokenVaultNativeUpgradeableInstance.address, 0, data, { from: owner });
        let txId = await getTxIdFromMultiSigWallet(multiSigWalletInstance);
        await multiSigWalletInstance.confirmTransaction(txId, { from: account1 });
        ...
        await localExpect(tokenVaultNativeUpgradeableInstance.transferFromVault(newSignature, amountToTransfer, { from: testAccountsData[1].address })).to.eventually.be.fulfilled;
    ...
    it('can be upgraded and store new state variables from the new contract', async () => {
        ...
        // Keep track of balances before the transfer
        let vaultBalanceBefore = (await web3.eth.getBalance(tokenVaultNativeUpgradeableInstance.address)).toString();
        const recipientBalanceBefore = (await web3.eth.getBalance(account1)).toString();

        // The amount and tx sender must match those that have been signed for
        await localExpect(tokenVaultNativeUpgradeableInstance.transferFromVault(signature, amountToTransfer, { from: testAccountsData[1].address })).to.eventually.be.fulfilled;

        let vaultBalanceAfter = (await web3.eth.getBalance(tokenVaultNativeUpgradeableInstance.address)).toString();
        const recipientBalanceAfter = (await web3.eth.getBalance(account1)).toString();

        expect(bigInt(vaultBalanceAfter).subtract(vaultBalanceBefore).toString()).to.equal(bigInt(amountToTransfer).multiply(bigInt('-1')).toString());
        expect(bigInt(recipientBalanceAfter).greater(recipientBalanceBefore)).to.be.true;

        tokenVaultNativeUpgradeableTestInstance = (await upgradeProxy(
            tokenVaultNativeUpgradeableInstance.address,
            TokenVaultNativeUpgradeableTest as any
        )) as TokenVaultNativeUpgradeableTestInstance;
        ...
        vaultBalanceBefore = (await web3.eth.getBalance(tokenVaultNativeUpgradeableInstance.address)).toString();
        const recipient2BalanceBefore = (await web3.eth.getBalance(account2)).toString();

        // The amount and tx sender must match those that have been signed for
        await localExpect(tokenVaultNativeUpgradeableInstance.transferFromVault(signature2, amountToTransfer, { from: testAccountsData[2].address })).to.eventually.be.fulfilled;

        vaultBalanceAfter = (await web3.eth.getBalance(tokenVaultNativeUpgradeableInstance.address)).toString();
