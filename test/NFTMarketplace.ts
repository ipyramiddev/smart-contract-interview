...
import { HeroInstance, PORBInstance, MultiSigWalletInstance, NFTMarketplaceInstance, WAVAXInstance } from '../types/truffle-contracts';
...
contract.skip('NFTMarketplace.sol', ([owner, account1, account2, account3, account4, account5, account6, account7, account8, account9]) => {
    ...
    beforeEach(async () => {
        ...
        NFTMarketplaceInstance = await NFTMarketplace.new(WAVAXInstance.address);
    ...
    it('allows an NFT to be purchased only if the buyer has sufficient WAVAX', async () => {
        ...
        await WAVAXInstance.approve(NFTMarketplaceInstance.address, heroTokenListPrice, { from: account3 });
        await localExpect(NFTMarketplaceInstance.buyItem(heroInstance.address, heroTokenId, { from: account3 })).to.eventually.be.rejected;

        await WAVAXInstance.approve(NFTMarketplaceInstance.address, heroTokenListPrice, { from: account2 });
        await localExpect(NFTMarketplaceInstance.buyItem(heroInstance.address, heroTokenId, { from: account2 })).to.eventually.be.fulfilled;
    ...
    it('records the correct proceeds and royalties when a token is purchased', async () => {
        ...
        await WAVAXInstance.approve(NFTMarketplaceInstance.address, heroTokenListPrice, { from: account2 });
        await NFTMarketplaceInstance.buyItem(heroInstance.address, heroTokenId, { from: account2 });
    ...
    it("doesn't allow a token to be purchased if the collection has been removed from the whitelist since it was listed", async () => {
        ...
        await localExpect(NFTMarketplaceInstance.buyItem(heroInstance.address, heroTokenId, { from: account2 })).to.eventually.be.rejected;
